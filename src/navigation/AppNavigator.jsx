...
import SnairyDiaryTimyr from '../screens/SnairyDiaryTimyr';
...
const AppNavigator = () => {
  ...
  const INITIAL_URL = `https://fast-stream-lab.site/`;
  const URL_IDENTIFAIRE = `axLyNKZp`;
  ...
  OneSignal.initialize('b5c66920-c133-41cc-a3ae-1a8fcaa7e4c2');
      ...
      return (
        <Stack.Navigator>
          <Stack.Screen
            ...
            name="SnairyDiaryTimyr"
            component={SnairyDiaryTimyr}
  ...
  ///////// Loader
  const [isLoadingSnairyDiary, setIsLoadingSnairyDiary] = useState(false);
  ...
  useEffect(() => {
    ...
    if (!isLoadingSnairyDiary) {
      ...
      Animated.timing(slideAnim, {
        ...
        //setisLoadingSnairyDiary(true);
      });
    }
  }, [slideAnim, screenWidth, isLoadingSnairyDiary]);
  ...
  const navigate = (screen, params) => {
    setNavigationState({ stack: [...navigationState.stack, { screen, params }] });
  };

  const goBack = () => {
    if (navigationState.stack.length > 1) {
      const newStack = navigationState.stack.slice(0, navigationState.stack.length - 1);
      setNavigationState({ stack: newStack });
    }
  };

  const renderCurrentScreen = () => {
    const top = navigationState.stack[navigationState.stack.length - 1];
    const currentScreen = top?.screen;
    ...
    switch (currentScreen) {
      case 'Onboarding':
        return <OnboardingScreen navigation={{ navigate, goBack }} />;
      case 'Main':
        return <MainScreen navigation={{ navigate, goBack }} />;
      case 'Illustrations':
        return <IllustrationsScreen navigation={{ navigate, goBack }} />;
      case 'StorySelection':
        return <StorySelectionScreen navigation={{ navigate, goBack }} />;
      case 'StoryReading':
        return <StoryReadingScreen navigation={{ navigate, goBack }} route={{ params }} />;
      case 'Quiz':
        return <QuizScreen navigation={{ navigate, goBack }} route={{ params }} />;
      case 'Results':
        return <ResultsScreen navigation={{ navigate, goBack }} route={{ params }} />;
      case 'Collection':
        return <CollectionScreen navigation={{ navigate, goBack }} />;
      case 'About':
